...
- (EvoUI *) initWithSize:(CGSize)size
{
    ...
    if (self) {
        ...
        _hungerBar = [[SKSpriteNode alloc] initWithColor:[SKColor blueColor] size:CGSizeMake(size.width/3 - 20,size.height/10)];
        [_hungerBar setAnchorPoint:CGPointMake(0.5,0.5)];
        [_hungerBar setAlpha:0.8];
        [_hungerBar setPosition:CGPointMake(0, size.height*9/20-8)];
        [_hungerBar addChild:[barBackground copy]];
        
        _energyBar = [[SKSpriteNode alloc] initWithColor:[SKColor yellowColor] size:CGSizeMake(0,size.height/10)];
        [_energyBar setAnchorPoint:CGPointMake(0.5,0.5)];
        [_energyBar setAlpha:0.8];
        [_energyBar setPosition:CGPointMake(size.width/3, size.height*9/20-8)];
        [_energyBar addChild:[barBackground copy]];
        
        [self addChild:_healthBar];
        [self addChild:_hungerBar];
...
- (void) redraw:(CGSize) oldSize
{
    ...
    [_hungerBar setXScale:[_hungerBar xScale]*size.width/oldSize.width];
    [_hungerBar setYScale:[_hungerBar yScale]*size.height/oldSize.height];
    [_hungerBar setPosition:CGPointMake(0, size.height*9/20-8)];
